...
//! `proc-macros`.
//!
//! This crate is the facade end users depend on. It re-exports the `Schema`
//! trait, the `Schemas` builder, and the `#[derive(Schema)]` macro so that
//! `use frieze::Schema;` brings both the trait and the derive into scope.

pub use frieze_macros::Schema;
pub use frieze_usecase::{BuildError, BuiltSchemas, Schema, Schemas};
