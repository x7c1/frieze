//! `Option<User>` (serde default — no `skip_serializing_if`) produces
//! the required + nullable shape (`Option<U>` serde default in
//! `docs/field-shapes/composite-shapes.md`). Under OAS 3.0, a nullable reference is
//! expressed with `allOf` + `nullable: true` because the spec disallows
//! sibling keys on a `$ref` schema.

use frieze::Schema;

mod common;

#[derive(Schema)]
#[allow(dead_code)]
struct User {
    id: i64,
}

#[derive(Schema)]
#[allow(dead_code)]
struct Profile {
    user: Option<User>,
}

#[test]
fn option_nested_renders_as_nullable_ref_under_oas_3_0() {
    let s: frieze_model::Schemas = frieze::SchemasBuilder::new()
        .add::<Profile>()
        .add::<User>()
        .build()
        .expect("schemas build should succeed for valid input");

    insta::assert_snapshot!(common::snapshot_yaml_3_0(s), @"
    openapi: X.Y.Z
    info:
      title: snapshot test
      version: 0.0.0
    components:
      schemas:
        Profile:
          type: object
          required:
          - user
          properties:
            user:
              allOf:
              - $ref: '#/components/schemas/User'
              nullable: true
        User:
          type: object
          required:
          - id
          properties:
            id:
              type: integer
              format: int64
    ");
}
